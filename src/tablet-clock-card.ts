...
import {
  type TabletClockCardConfig,
  type MergedTabletClockCardConfig,
...
export class TabletClockCard extends LitElement {
  ...
  @state() private config!: MergedTabletClockCardConfig
    ...
    return html`
      ...
      >
        ...
        <div class="card-content">
          ${showToday
        ? html`
            <tablet-clock-card-today>
              ${safeRender(() => this.renderToday())}
            </tablet-clock-card-today>`
        : ''}
          ${showForecast
        ? html`
            <tablet-clock-card-forecast>
  ...
  private renderToday (): TemplateResult {
    ...
    return html`
      <tablet-clock-card-today-left>
        <img class="grow-img" src=${icon} />
      </tablet-clock-card-today-left>
      <tablet-clock-card-today-right>
        <tablet-clock-card-today-right-wrap>
          <tablet-clock-card-today-right-wrap-top>
          ...
          <tablet-clock-card-today-right-wrap-center>
            ${this.config.hide_clock ? localizedTemp ?? 'n/a' : this.time()}
          </tablet-clock-card-today-right-wrap-center>
          <tablet-clock-card-today-right-wrap-bottom>
  ...
  private renderForecastItem (forecast: MergedWeatherForecast, minTemp: number, maxTemp: number, currentTemp: number | null, temperatureUnit: TemperatureUnit, hourly: boolean, displayText: string, maxColOneChars: number): TemplateResult {
    ...
    return html`
      <tablet-clock-card-forecast-row style="--col-one-size: ${(maxColOneChars * 0.5)}rem;">
      ...
      </tablet-clock-card-forecast-row>
    `
  }
  ...
  private mergeConfig (config: TabletClockCardConfig): MergedTabletClockCardConfig {
    return {
      ...config,
  ...
  private toZonedDate (date: DateTime): DateTime {
    ...
    console.error(`tablet-clock-card - Time Zone [${timeZone}] not supported. Falling back to browser time.`)
  ...
  private async subscribeForecastEvents (): Promise<void> {
    ...
    try {
      ...
      console.error('tablet-clock-card - Error when subscribing to weather forecast', e)
    } finally {
      this.forecastSubscriberLock = false
    }
  }
  ...
  private determineForecastType (): 'hourly' | 'daily' | 'hourly_not_supported' {
    ...
    if (supportsDaily && supportsHourly) {
      ...
      console.warn(`tablet-clock-card - Weather entity [${this.config.entity}] does not support daily forecast. Falling back to hourly forecast.`)
      return 'hourly'
    }
  }
